...
#############################
# Saving the ci climatology for BSS computation 
if OPTION == 3:
    ...
    for m in tqdm(range(1, 13), desc='Monthly climatology'):
        ...
        ds_clim = add_lat_lon(clim_meanx[6:], ['ci'])  # Only save the CI variable (the last one), and add lat/lon coordinates for saving as netcdf
        ds_clim.to_netcdf(f'~/scratch_folder/final/processed/ci_clim_{m:02d}.nc')  # Save each month's climatology separately to avoid memory issues
    ...
    clims.to_netcdf(f'~/scratch_folder/final/processed/ci_clim_monthly.nc')
